...
一个人也能开一家公司。这个项目把「想需求、写内容、做运营、看数据、管账、做产品、写代码」这些活，拆给一整套 AI 角色（R0~R9）一起干。opc-web 就是这家公司的**指挥台**——浏览器里打开就能管理这一整套流程。它分成两层：
...
- **「当前:」实时执行代号**：组织架构每张角色卡显示「当前:」——正在执行的任务代号（如 `T-001`），没有则显示「无」，一眼看清谁在忙。
- **一键直达角色产物**：组织架构角色卡新增「文件夹」按钮，点击一键跳到「项目文件」视图，并自动按该角色筛选其工作区产物。
- **共享技能库 + Skill 导入**：技能 md 平铺共享在 `agents/skills/`，一份技能多角色复用；设置页「Skill 导入」可从 dsh 已装技能（`~/.dsh/skills/` + 各 profile 插件自带，如 ponytail / archify / frontend-design / investment-research 等）**一键导入**，支持搜索。
- **深浅双主题**：右上角一键切换浅色（电青）/ 深色（鎏金），选择记忆；界面强调「只留标题、去冗余说明」，配色与布局深浅完全对齐。
...
- **智能任务看板**：子任务按状态四列展示（待派 / 执行中 / 已完成 / 已归档），支持搜索、按任务筛选执行角色、点击任务行 / 看板卡片聚合全程输出。
- **公文式批阅台 + 决策闭环**：待决条目只放「项目整体进展 / 需要 R0 拍板什么 / R1 建议」；R0 盖**批准**章 → 控制台自动新建「执行 R0 决策」任务交 R1 派发执行；**驳回 / 修改** → 执行角色按批注补写后重新回报。
- **Token 用量统计**：子任务 headless 执行时把 dsh 用量写回各自 `.meta.json`，设置页 Token 统计按任务聚合展示。
- **产出文件规范化命名**：`工作区/<角色>/T-001-S1-report.md` → 归档自动改名 `-output.md` 连元数据移入 `已归档/`；R1 任务汇总 `T-xxx-summary.md`。
- **定时任务**：每天 / 每周 / 每隔 N 分钟，到点自动落台账生成调度指令，与手动下达同链路，支持启停 / 删除。
- **知识库沉淀**：决策日志、调度记录、知识档案统一归档，R1 汇总生成每日简报，知识随项目走。
- **状态与正文分层存储**：任务 / 子任务 / 回报状态在 SQLite（唯一真相），正文与公文仍是人读人写的 md。
