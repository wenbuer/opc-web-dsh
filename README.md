...
- **Token 用量统计**：子任务 headless 执行时把 dsh 用量（输入含缓存读取 / 输出 / 缓存读取 / 推理）写回各自 `.meta.json`，设置页 ④ Token 统计按任务聚合并柱状展示——执行成本摊到明面上（仅改版后执行的任务计入，旧任务不计）。
- **产出文件规范化命名**：执行中的子任务交付物 = `工作区/<角色>/T-001-S1-report.md`（完成回报，进度可边做边追加）；归档时自动改名 `-output.md` 连元数据移入 `工作区/<角色>/已归档/`；R1 任务汇总 = `T-xxx-summary.md`；历史旧命名（`T-001-S1.md` / `T-xxx-工作汇总.md`）自动迁移改名。
- **项目文件视图**：按角色 / 任务浏览各角色工作区产物，md 渲染查看、其他文本以 txt 预览；二进制 / 不可解码 / 超 3 MB 的文件不提供在线预览，路径越界一律拦截。
...
4. **执行回报**：角色 / headless 把进度边做边追加进 `T-001-S1-report.md`，完成后把 `.meta.json` 的 `status` 置为 完成 / 部分 / 阻塞；自动执行还会把当次 token 用量（输入含缓存读取 / 输出等）一并写回 meta；
5. **汇总批阅**：R1 汇总各子任务回报生成 `工作区/老板助理（枢纽）/T-001-summary.md` 并整理成待决条目 → R0 在批阅台裁决（见[决策闭环](#r0-批阅与决策闭环)）——批准即自动新建「执行 R0 决策」任务继续派发；
6. **归档**：点「🗂 归档」→ 控制台把已完成子任务的 `-report.md` 改名 `-output.md`、连同 `.meta.json` 移入 `工作区/内容工厂/已归档/`，回报入库，子任务全完成时任务自动置「完成」；工作台任务详情可回看全程，设置 ④ 可看每任务 token 用量。
...
| 子任务元数据 | `工作区/<角色>/T-xxx-Sn.meta.json`：状态（子 agent 回填）+ 执行 token 用量（自动执行时写入 `tokensIn/tokensOut/tokensCacheRead/tokensReasoning`），设置页 ④ 汇总展示 |
