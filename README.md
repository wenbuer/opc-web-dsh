...
- **R0**：下达任务、批阅角色产出、归档决策；
- **R1**：读取台账 / 日志 → 拆分任务 → 以 subagent 派发至对应角色；
...
1. **下达**：老板在作战面板 / 工作台下达任务 → 任务写入台账；
2. **拆解派发**：R1 将任务拆为子任务，派发给各角色执行（工作台为执行与展示载体）；
3. **执行回报**：角色执行并将结果回报至台账；
4. **汇总**：R1 汇总各角色回报——当日有任务则生成每日简报；同时把需要老板拍板的事项整理为待决条目；
5. **批阅**：老板在批阅台拍板（批准 / 驳回 / 修改）；
6. **提炼与再派发**：R1 从任务产出中抽取必要内容沉淀进知识库；已批准事项作为新任务继续下达给角色，回到拆解派发，形成闭环。
