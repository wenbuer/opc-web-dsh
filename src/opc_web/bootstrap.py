...
def bootstrap():
    ...
    seeds = {
        ...
        config.INDEX_REL: "# 知识库索引" + h + h + "目录模型 v1.10：单根目录 → 批阅台/、工作区/（按角色名称）、知识库/。" + h + "角色卡三通道影响 dsh：agent-preset（~/.dsh/.agent-presets/opc-r?）/ persona / prompt 注入。" + h,
        config.DISPATCH_REL: "| 任务编号 | 子任务 | 承接角色 | 期望产出 | 状态 |" + h + "|---|---|---|---|---|" + h,
        config.LOG_REL: "## 决策日志" + h,
        config.PIYUETAI_REL: "## 待决" + h + "| 编号 | 待决事项 | 提出者 | 日期 | R0 批阅 |" + h + "|---|---|---|---|---|" + h + h + "## 已批阅归档" + h + "| 编号 | 事项 | 批阅 | 日期 |" + h + "|---|---|---|---|" + h,
        config.REPORT_REL: "| 日期 | 任务号 | 角色 | 标题 | 状态 | 记录 |" + h + "|---|---|---|---|---|---|" + h,
    }
